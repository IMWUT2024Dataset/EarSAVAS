...
def calculate_stats(output, target):
    ...
    confusion_matrix = np.zeros((classes_num, classes_num), dtype = np.int32)

    _, target_i = torch.max(target.data, 1)
    _, output_i = torch.max(output.data, 1)
    target_i = target_i.cpu().numpy()
    output_i = output_i.cpu().numpy()

    np.add.at(confusion_matrix, (target_i, output_i), 1)
